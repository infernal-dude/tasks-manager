...
// Непонятно из названия что это за структура.
// Это не структура сервера, потому что сервер стартует отдельно, хотя хранит в себе хендлеры этого сервера.
// Причём есть ещё зеркальная структура для юзеров. Наличие TaskHandler и UserHandler путает, т.к. непонятно зачем они нужны.
